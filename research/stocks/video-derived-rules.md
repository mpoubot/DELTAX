# Stocks — video-derived rules (SUPERSEDED, reference only)

> ⚠️ **Superseded by [`golden-rules.md`](golden-rules.md)** — Elsa's Alyrise spec is
> the authoritative stock strategy. This file is retained because its risk and
> process layer (S0–S6) informed `deltax/gates.py`, which serves the **options**
> engine. Do not apply these to the stock engine.
