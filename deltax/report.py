...
C = {"g": "\033[92m", "r": "\033[91m", "y": "\033[93m", "c": "\033[96m",
     "d": "\033[90m", "w": "\033[97m", "b": "\033[1m", "x": "\033[0m"}
...
def header(equity, cash, market_open, now=None):
    ...
    return (f"{C['c']}┌─ {now:%a %H:%M} ET{C['x']} │ {state} │ "
...
def scoreboard(equity, realized, unrealized):
    ...
    return (f"{C['c']}│{C['x']} {C['b']}SCOREBOARD{C['x']}  "
...
def position_line(p):
    ...
    bits = [f"{C['c']}│{C['x']} {C['w']}{p['symbol']:<5}{C['x']} ✋ {C['y']}HOLDING{C['x']} "
...
def event(kind, sym, text):
    ...
    return f"{C['c']}│{C['x']} {C[col]}[{sym}]{C['x']} {icon} {C[col]}{text}{C['x']}"
...
def refusal_summary(refused, limit=6):
    ...
    for gate, names in sorted(by.items(), key=lambda kv: -len(kv[1])):
        ...
        out.append(f"{C['c']}│{C['x']} {C['d']}⛔ {len(names):>2} × {gate:<16}{C['x']} "
...
def render(*, equity, cash, market_open, realized=0.0, unrealized=0.0,
    ...
    if not positions and not events:
        lines.append(f"{C['c']}│{C['x']} {C['d']}no open positions{C['x']}")
