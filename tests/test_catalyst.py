...
class FakeFeed:
    """Controllable book. Defaults mirror the live 1 Sep quotes."""
    def __init__(self, long_ask=1.12, long_bid=1.07, short_bid=0.31,
                 short_ask=0.35, long_oi=7745, short_oi=8087, listed=True,
                 raise_on=False):
        self.__dict__.update(locals()); del self.self
    def option_contracts(self, u, **kw):
        if self.raise_on: raise RuntimeError("feed down")
        return [{"symbol": "USO260904C00145000", "open_interest": str(self.long_oi)},
                {"symbol": "USO260904C00150000", "open_interest": str(self.short_oi)}]
    def option_chain(self, u, **kw):
        if self.raise_on: raise RuntimeError("feed down")
        if not self.listed: return {}
        return {
            "USO260904C00145000": {"latestQuote": {"ap": self.long_ask, "bp": self.long_bid}},
            "USO260904C00150000": {"latestQuote": {"ap": self.short_ask, "bp": self.short_bid}},
        }


print("\n── pricing: pay the ask, receive the bid ──")
v = price_vertical(FakeFeed())
check("prices at ask minus bid, never mid", v.debit == 0.81, str(v.debit))
check("accepted at a debit inside the ceiling", v.ok, v.reason)
check("breakeven is long strike plus debit", v.breakeven == 145.81, str(v.breakeven))
check("exit rests at 3x the debit", v.exit_limit == 2.43, str(v.exit_limit))

print("\n── sizing derives from the ACTUAL debit, never a fixed count ──")
check("$0.81 -> 123 contracts", v.contracts == 123, str(v.contracts))
check("risk is the debit paid", v.max_loss == 9963.0, str(v.max_loss))
check("max profit uses width minus debit",
      v.max_profit == round((5.0 - 0.81) * 100 * 123, 2), str(v.max_profit))
v90 = price_vertical(FakeFeed(long_ask=1.21))          # debit 0.90
check("$0.90 -> 111 contracts", v90.contracts == 111, str(v90.contracts))
v100 = price_vertical(FakeFeed(long_ask=1.31))         # debit 1.00
check("$1.00 (the ceiling) -> 100 contracts", v100.contracts == 100, str(v100.contracts))
check("risk never exceeds the budget",
      all(x.max_loss <= RISK_BUDGET for x in (v, v90, v100)),
      f"{v.max_loss}, {v90.max_loss}, {v100.max_loss}")

print("\n── every refusal path ──")
over = price_vertical(FakeFeed(long_ask=1.32))         # debit 1.01
...
wide = price_vertical(FakeFeed(long_ask=3.00, long_bid=0.10))
check("a wide book refuses", not wide.ok, wide.reason)
crossed = price_vertical(FakeFeed(long_ask=0.20, short_bid=0.31))
...
print("\n── open-interest cap ──")
capped = price_vertical(FakeFeed(long_oi=1000, short_oi=1000))
check("size is capped at 5% of the thinner leg",
      capped.contracts <= int(1000 * MAX_OI_FRACTION), str(capped.contracts))
check("the cap is recorded, not silent", "oi_capped" in capped.detail, str(capped.detail))

print("\n── execution legs ──")
lg = legs_for(v)
check("two legs", len(lg) == 2)
check("buys the low strike", lg[0].symbol.endswith("00145000") and lg[0].side == "buy")
check("sells the high strike", lg[1].symbol.endswith("00150000") and lg[1].side == "sell")
